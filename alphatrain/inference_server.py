"""Centralized GPU inference server for parallel MCTS (policy-only).
...
Architecture:
    ...
    GPU reads from shared memory, batches, evaluates policy ->
    writes policy logits to shared memory -> signals worker via response queue.
...
class InferenceClient:
    ...
    __slots__ = ('slot_id', 'obs_buf', 'pol_buf',
                 'request_queue', 'response_queue')

    def __init__(self, slot_id, obs_buf, pol_buf,
                 request_queue, response_queue):
    ...
    def evaluate(self, obs_np):
        """Send single observation, wait for policy_logits.

        Args:
            obs_np: numpy array (18, 9, 9) float32

        Returns:
            policy_logits: np.ndarray (6561,)
        """
    ...
    def evaluate_batch(self, obs_batch_np, count):
        ...
        Returns:
            pol_np: view into shared memory (valid until next call)
        """
...
class InferenceServer:
    ...
    def __init__(self, model_path, num_workers, device=None,
                 max_batch_per_worker=MAX_BATCH,
                 deterministic=False):
        ...
        # Shared memory: obs[N, B, 18, 9, 9], pol[N, B, 6561]
    ...
    def start(self):
        ...
        self._process = Process(
            ...
            args=(self.model_path, self.device_str,
                  ...
                  self._obs_shm.name, self._pol_shm.name,
    ...
    def make_client(self, slot_id):
        ...
        return InferenceClient(
            slot_id, self.obs_buf, self.pol_buf,
    ...
    def shutdown(self):
        ...
        for shm in (self._obs_shm, self._pol_shm):
            shm.close()
            shm.unlink()
...
def _gpu_loop(model_path, device_str, num_workers, max_batch,
              ...
              deterministic=False):
    """GPU inference process main loop (policy-only).
    ...
    print(f"GPU inference server ready ({device_str}, fp16+jit, "
          f"{num_workers} slots, max_batch={max_batch}, policy-only)", flush=True)
