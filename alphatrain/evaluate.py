...
def load_model(model_path, device, fp16=False, jit_trace=False):
    """Load PolicyNet from checkpoint, auto-detecting architecture.

    Args:
        fp16: convert to half precision (2x faster on MPS/CUDA)
        jit_trace: apply torch.jit.trace (10-15% faster forward pass)

    Returns (net, max_score) where max_score is metadata kept around for
    downstream tooling — not used by policy-only models in MCTS.

    Old dual-head checkpoints (pre-V10: pillar2u/2v/2w/2w2) include
    value_* keys in their state_dict. We filter those out so they load
    into PolicyNet via strict=False semantics.
    """
...
def make_policy_player(net, device):
    """Greedy argmax policy player from PolicyNet."""
    try:
...
def main():
    ...
    p.add_argument('--feature-value-weights', default=None,
                   help='Required for MCTS: feature_value_weights.npz')
    args = p.parse_args()
