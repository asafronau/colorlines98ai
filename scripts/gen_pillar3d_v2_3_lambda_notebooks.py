...
**This run: same DECISIVE corpus (v2.3 filter, 11,582 corrections) + gentler `--aux-lambda {lam}`**
...
1. `colorlines_pillar3d_v2.tar.gz` — code (re-upload if unsure; needs the `--grad-audit`-era trainer? no — recipe flags unchanged)
2. **`corrections_corpus_mm05.pt`** — the 11,582-decisive corpus (~3.8 MB)
...
assert 9000 < _c['boards'].shape[0] < 14000 and _c['_stats']['min_margin'] >= 0.05, \
    "expected the ~11.6k decisive corpus — upload corrections_corpus_mm05.pt"
t0 = time.time()
