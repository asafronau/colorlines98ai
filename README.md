...
| Policy + TASK-ARITHMETIC merge — ta15_α0.7 / pillar3f (5k seeds, no cap) | 31,617 | 22,181 | 93,129 | 1.6% | None (the deployed pillar3f baseline; α0.4 ties: 31,790/21,591) |
| **Policy + DECISIVENESS-weighted crisis distillation — pillar3k ep22** (5k seeds, no cap) | **43,390** | **31,016** | 126,379 | **1.3%** | None ← **new best, 5k-confirmed** (+37% mean / +40% median over pillar3f) |
...
Individual single-game scores have reached **389,307** (pillar3k, 5k eval) — many multiples of the long-standing 40K expert-human ceiling.

**Crisis corrections + task arithmetic** (HISTORY 165-167) produced the pillar3f baseline: mine the policy's death games, rewind to the crisis band, label states with widened MCTS@4800 ("grandmaster moves" — human-validated in the GUI), then — instead of mixing a small auxiliary loss into re-distillation (which throttled the gains to +25% via gradient competition) — fine-tune a copy ONLY on the corrections (frozen BN) and merge weights: θ_deploy = θ_base + α·(θ_crisis − θ_base), sweeping α with a cheap local eval. The same correction corpus that gave +3.3k median through the aux channel gives **+5.7k median / +36% mean over that channel's best** through the merge (5k-confirmed, equal floor). Modern evals are policy-only, no turn cap, fixed seed lists, per-seed scores saved for paired comparisons.

**The current best is decisiveness-weighted crisis distillation — pillar3k** (HISTORY 173-174). Every prior attempt to distill a *fresh* crisis corpus back into the strong pillar3f policy *de-peaked* it and regressed: the crisis + self-play corpus is dominated by flat "many-moves-are-fine" quiet positions, and no target-temperature can sharpen a flat corpus into an already-peaked policy (measured: even T=0.25 only reaches visit top-share 0.30 vs pillar3f's 0.40). The fix weights each state's loss by its *decisiveness* — `(visit top-share)^P` via `train_path_b --decisiveness-power 3.0` — so the rare escape-or-die states (one move dominates) drive the gradient at ~6.6× weight while flat quiet states get ~0.15× and **cannot flatten the policy**. Distilling a 70/30 crisis/self-play corpus this way, warm-started from pillar3f, yields **pillar3k = mean 43,390 / median 31,016 / P10 5,010 / %<1000 1.3%** (5k-confirmed) — the first full-corpus distillation to beat pillar3f (**+37% mean / +40% median**), lifting the floor *without* flattening typical play. Hard-won lessons: pick the checkpoint by gameplay floor **mid-training** (late epochs trade floor for ceiling); the val loss is **unreliable** under this weighting (it tracks the de-emphasized flat states, not gameplay); and **5,000-seed** evals are required to resolve sub-10% differences that 500 seeds wash out.
